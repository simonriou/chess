...
def compare_models(models):
    ...
    Also exports the performances (Loss, MAE) of the models to JSON file (append, not overwrite).
    ...
    for i, model in enumerate(models):
        ...
        performances[f"Model {i + 1} (single)"] = {
            "Test Loss": test_loss,
            "Test MAE": test_mae
        }
    ...
    # Export the performances to a JSON file
    with open(f"performances/single_vs_double.json", 'a') as f:
...
def train_model(X_train, y_train, epochs=50, batch_size=64):
    """
    Trains a CNN model on the training data and saves it to disk.
